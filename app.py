...
# --- 頁面設定 ---
st.set_page_config(page_title="莎拉爸貼圖神器 v6.5", page_icon="🐴", layout="wide")
st.title("🐴 莎拉爸貼圖神器 v6.5 (介面優化版)")
st.markdown("🚀 **v6.5 更新**：將執行按鈕移至主畫面，操作更直覺！")
...
if "智慧" in slice_mode:
    ...
    with c1:
        manual_rows = st.number_input("縱向列數 (Rows)", 1, 10, 3) # 預設改為 3 以配合最近的 4x3
    with c2:
        manual_cols = st.number_input("橫向行數 (Cols)", 1, 10, 4) # 預設改為 4
...
# --- 主程式區 (v6.5 修正：將按鈕移至此處) ---

if uploaded_files:
    # 顯示上傳檔案資訊
    st.info(f"📂 已上傳 {len(uploaded_files)} 個檔案。準備就緒！")
    
    # [v6.5 關鍵修改] 按鈕放在主畫面，不再是 sidebar
    if st.button("🚀 開始處理圖片 (Start Processing)", type="primary", use_container_width=True):
        st.session_state.processed_stickers = []
        st.session_state.original_images = []
...
if st.session_state.processed_stickers:
    ...
    st.download_button(
        ...
        file_name="SarahDad_Stickers_v6.5.zip",
